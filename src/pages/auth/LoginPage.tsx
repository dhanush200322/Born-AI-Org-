...
import { MoveRight, Mail, Lock, Eye, EyeOff, CheckCircle2 } from "lucide-react";
    ...
    <PageTransition>
      <AuthLayout>
        <motion.div
          ...
          className="p-8 sm:p-10 rounded-2xl border border-[#1a1f2e] bg-[#0a1020]/50 backdrop-blur-xl shadow-[0_0_40px_-15px_rgba(79,140,255,0.1)] relative"
        >
          <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent rounded-2xl pointer-events-none" />

          <div className="relative z-10 mb-8">
            <h2 className="text-2xl font-bold mb-2">Welcome Back</h2>
            <p className="text-gray-400 text-sm">Sign in to manage your AI agents.</p>
          </div>

          <form onSubmit={handleSubmit} className="relative z-10 space-y-5">
            {successMessage && (
              <div className="bg-[#4F8CFF]/10 border border-[#4F8CFF]/20 text-[#4F8CFF] text-sm p-3 rounded-xl">
                {successMessage}
              </div>
            )}
            {error && (
            ...
            <div className="flex items-center justify-between text-sm pt-2">
              <label className="flex items-center gap-2 cursor-pointer group">
                <div className="relative flex items-center justify-center w-4 h-4 rounded border border-[#1a1f2e] bg-[#04070D] group-hover:border-[#4F8CFF]/50 transition-colors">
              ...
              <Link to="/forgot-password" className="text-[#4F8CFF] hover:text-white transition-colors">
                Forgot password?
              </Link>
            </div>
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-gradient-to-r from-[#4F8CFF] to-[#9F5CFF] hover:from-[#4F8CFF]/90 hover:to-[#9F5CFF]/90 text-white font-medium py-2.5 rounded-xl transition-all flex items-center justify-center gap-2 group shadow-[0_0_20px_-5px_rgba(79,140,255,0.4)] disabled:opacity-70"
            >
              {isLoading ? (
        ...
        <p className="text-center text-gray-500 text-sm mt-8 relative z-10">
          ...
          <Link to="/signup" className="text-[#4F8CFF] hover:text-white font-medium transition-colors">
            Create Account
          </Link>
        </p>
      </AuthLayout>
